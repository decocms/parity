...
| **Lighthouse opportunities** _(#264)_ | `parity vitals` (Lighthouse mode): the actionable audits Lighthouse already computes — render-blocking resources, lazy-loaded LCP image, unused preconnect, unused JS/CSS, `font-display`, `legacy-javascript`… — as issues, severity from the estimated savings (≥500ms high, ≥200ms medium, below that counted but not filed). An audit prod does not trigger, or wastes ≥200ms less time on, is flagged as introduced by the migration | perf |
...
| **SSR / no-JS** _(issue #244)_ | Plain `fetch` (no JS) returns the SSR HTML; a near-empty body → content is client-only and the page is blank without JS (high CLS, broken SEO/a11y). Against a **localhost candidate** the finding is kept but marked `inconclusive` — several frameworks render client-side in dev, so it must be reconfirmed on a production build before counting as a defect (#292) | html |
