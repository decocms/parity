...
| `parity plan board` | Per-page kanban: every sampled page in a derived lane (triage \| backlog \| building \| review \| done) plus what blocks it (`--cand`, `--json`). `--board studio` mirrors one card per page into the deco Studio task board (`--repo`, `--fixes`) |
| `parity plan notes` | Read the client's comments off the Studio cards (the input channel), or `--post` a confirmation back (`--page`, `--json`) |
